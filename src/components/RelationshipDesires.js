...
const RelationshipDesires = ({ contact, isMyProfile = false, onUpdate }) => {
  ...
  const businessSupports = [
    'Strategic Advice', 'Business Connections', 'Investment Opportunities', 
    'Partnership Introductions', 'Market Insights', 'Problem Solving',
    'Accountability', 'Brainstorming', 'Industry Knowledge', 'Skill Development'
  ];

  // Personal support options
  const personalSupports = [
    'Clarity & Perspective', 'Fun & Humor', 'Emotional Support', 'Motivation',
    'Honest Feedback', 'Creative Inspiration', 'Life Balance', 'Adventure Planning',
    'Deep Conversations', 'Celebration Partner'
  ];

  // My strengths (what I can give)
  const myStrengths = [
    'Business Strategy', 'Real Estate Expertise', 'Coaching Skills', 'Tourism Knowledge',
    'App Development', 'Networking', 'Leadership', 'Investment Advice', 'Marketing',
    'Operations', 'Team Building', 'Innovation'
  ];

  // Resources I can share
  const resources = [
    'Business Contacts', 'Investment Opportunities', 'Industry Reports', 'Tools & Software',
    'Event Invitations', 'Learning Resources', 'Vendor Recommendations', 'Office Space',
    'Equipment Access', 'Travel Connections'
  ];

  // Short-term relationship goals
  const shortTermGoals = [
    'Monthly Strategy Sessions', 'Weekly Check-ins', 'Quarterly Adventures',
    'Business Collaboration', 'Skill Exchange', 'Project Partnership',
    'Investment Together', 'Travel Together', 'Learn Together'
  ];

  // Long-term relationship goals
  const longTermGoals = [
    'Lifelong Business Partners', 'Investment Partners', 'Family Friends',
    'Mastermind Co-founders', 'Board Members', 'Mentorship Relationship',
    'Legacy Building Together', 'Retirement Planning Buddies'
  ];
      ...
      <div className="relationship-desires card">
        <div className="desires-header">
          ...
          <button onClick={() => setEditMode(true)} className="edit-desires-button">
            {Object.keys(desires.whatIWant.businessSupport || {}).length > 0 ? 'Edit' : 'Set Up'} Relationship Goals
          </button>
        </div>

        {desires.whatIWant.businessSupport?.length > 0 || desires.whatIWant.personalSupport?.length > 0 ? (
            ...
            <div className="desire-section">
              ...
              <div className="desire-tags">
                {desires.whatIWant.businessSupport?.map(item => (
                  <span key={item} className="desire-tag business">{item}</span>
                ))}
                {desires.whatIWant.personalSupport?.map(item => (
                  <span key={item} className="desire-tag personal">{item}</span>
                ))}
              </div>
              ...
              <div className="desire-section">
                ...
                <div className="desire-tags">
                  {desires.whatICanGive.myStrengths?.map(item => (
                    <span key={item} className="desire-tag giving">{item}</span>
                  ))}
                </div>
              ...
              <div className="desire-section">
                ...
                <div className="desire-tags">
                  {desires.relationshipGoals.shortTerm?.map(item => (
                    <span key={item} className="desire-tag goals">{item}</span>
                  ))}
                </div>
                {desires.relationshipGoals.mastermindPotential && (
                  <div className="mastermind-indicator">
                    ⭐ Mastermind Potential: High
                  </div>
                )}
              </div>
            )}
          ...
          <div className="empty-desires">
            <p>🎯 Set up what you want from this relationship to unlock AI-powered optimization suggestions!</p>
              ...
              {resources.map(resource => (
                <label key={resource} className="checkbox-label">
                  <input
                    ...
                    onChange={() => handleArrayUpdate('whatICanGive', 'resourcesICanShare', resource)}
                  />
                  {resource}
                </label>
              ))}
        ...
        <div className="editor-section">
          ...
          <div className="subsection">
            <label>
              <input
                ...
                onChange={(e) => handleFieldUpdate('relationshipGoals', 'mastermindPotential', e.target.checked)}
              />
          ...
          <div className="subsection">
            ...
            <textarea
              ...
              onChange={(e) => handleFieldUpdate('relationshipGoals', 'mutualProjects', e.target.value)}
