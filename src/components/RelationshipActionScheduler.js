...
const RelationshipActionScheduler = ({ contacts, userProfile, onClose }) => {
  ...
  const generateActionSuggestions = () => {
    ...
    contacts.forEach(contact => {
      ...
      if (contact.dateAdded) {
        ...
        if (friendshipAnniversary > today && friendshipAnniversary <= new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000)) {
          actions.push({
  ...
  const getRecommendedFrequency = (contact) => {
    switch (contact.category) {
      case 'Inner Circle': return { days: 7, priority: 'high' };
      case 'Business': return { days: 14, priority: 'high' };
      case 'Regular Friends': return { days: 21, priority: 'medium' };
      case 'Network': return { days: 60, priority: 'low' };
      default: return { days: 30, priority: 'medium' };
    }
  };
  ...
  const generateNetworkActions = () => {
    ...
    if (contacts.length >= 5) {
      actions.push({
        ...
        description: 'Review your relationship portfolio balance and identify optimization opportunities.',
        estimatedDuration: 30,
    ...
    for (let i = 0; i < contacts.length; i++) {
      for (let j = i + 1; j < contacts.length; j++) {
        ...
        if (contact1.professionalInterests && contact2.theirExpertise) {
          if (contact1.professionalInterests.toLowerCase().includes(contact2.theirExpertise.toLowerCase().split(' ')[0])) {
            opportunities.push({ person1: contact1.name, person2: contact2.name, reason: 'professional synergy' });
          }
        }
        ...
        if (contact1.personalGoals && contact2.personalGoals) {
          ...
          if (goals1.includes('business') && goals2.includes('business')) {
            opportunities.push({ person1: contact1.name, person2: contact2.name, reason: 'similar goals' });
          }
        }
      }
    }
  ...
  const scheduleAction = (action, customDate = null, customTime = null) => {
    ...
    if (userProfile?.clerkUserId) {
      localStorage.setItem(`friendsync_calendar_${userProfile.clerkUserId}`, JSON.stringify(updatedEvents));
    }
            ...
            <div className="suggestions-tab">
              {selectedActions.length > 0 && (
                <div className="bulk-actions">
                  <button 
                    onClick={scheduleSelectedActions}
                    className="schedule-selected-btn"
                  >
              ...
              {suggestedActions.length === 0 && (
                <div className="no-suggestions">
                  ...
                  <p>No urgent relationship actions needed right now. Check back later for new suggestions.</p>
                </div>
              )}
            </div>
          )}
