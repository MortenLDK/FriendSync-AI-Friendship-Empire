...
• "When should I reach out to ${friend.name} next?"`
  ...
  const createFriendContext = () => {
    ...
    context.push(`You are a strategic relationship advisor for ${friend.name}. Here's everything I know about them:`);
    ...
    if (friend.loveLanguageSecondary) context.push(`Secondary Love Language: ${friend.loveLanguageSecondary}`);
    ...
    if (friend.professionalInterests) context.push(`Professional Interests: ${friend.professionalInterests}`);
    if (friend.learningGoals) context.push(`Learning Goals: ${friend.learningGoals}`);
    if (friend.entertainmentPrefs) context.push(`Entertainment Preferences: ${friend.entertainmentPrefs}`);
    if (friend.howWeMet) context.push(`How We Met: ${friend.howWeMet}`);
    if (friend.communicationFreq) context.push(`Preferred Communication Frequency: ${friend.communicationFreq}`);
    if (friend.bestSupportMethods) context.push(`Best Ways to Support Them: ${friend.bestSupportMethods}`);
    ...
    if (friend.collaborationOpps) context.push(`Collaboration Opportunities: ${friend.collaborationOpps}`);
  ...
  const generateOfflineResponse = (question, friend) => {
    ...
    // Gift suggestions
    if (lowerQuestion.includes('gift') || lowerQuestion.includes('birthday') || lowerQuestion.includes('present')) {
      let suggestions = [];
      if (friend.loveLanguage === 'Receiving Gifts') {
        suggestions.push("Since their love language is Receiving Gifts, they'll especially appreciate thoughtful presents!");
      }
      if (friend.hobbies) {
        suggestions.push(`Based on their hobbies (${friend.hobbies}), consider gifts related to these interests.`);
      }
      if (friend.learningGoals) {
        suggestions.push(`They're learning: ${friend.learningGoals} - educational gifts in these areas would be perfect.`);
      }
      return suggestions.length > 0 ? suggestions.join(' ') : "Consider something personal that shows you remember their interests and goals.";
    }
    
    // Communication advice
    if (lowerQuestion.includes('communicate') || lowerQuestion.includes('talk') || lowerQuestion.includes('reach out')) {
      let advice = [];
      if (friend.communicationStyle) {
        advice.push(`Their communication style is ${friend.communicationStyle}, so adjust your approach accordingly.`);
      }
      if (friend.loveLanguage === 'Words of Affirmation') {
        advice.push("Since their love language is Words of Affirmation, be generous with genuine compliments and encouragement.");
      }
      if (friend.communicationFreq) {
        advice.push(`They prefer ${friend.communicationFreq} communication frequency.`);
      }
      return advice.length > 0 ? advice.join(' ') : "Be authentic and consider their personality when reaching out.";
    }
    ...
    if (lowerQuestion.includes('support') || lowerQuestion.includes('help')) {
      ...
      if (friend.loveLanguage === 'Acts of Service') {
        support.push("Their love language is Acts of Service - offer practical help with tasks or projects.");
      }
      ...
      return support.length > 0 ? support.join(' ') : "Listen actively and offer help that matches their current needs.";
    }
    ...
    if (lowerQuestion.includes('collaborate') || lowerQuestion.includes('work together') || lowerQuestion.includes('partnership')) {
      ...
      if (friend.theirExpertise && friend.howICanHelp) {
        collab.push(`Combine their expertise (${friend.theirExpertise}) with how you can help (${friend.howICanHelp}).`);
      }
      return collab.length > 0 ? collab.join(' ') : "Look for projects where your skills complement each other.";
    }
